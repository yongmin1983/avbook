...
@section('content')
  ...
  <div class="container">
    ...
    if ($movie_info['owned'] == 1) {
        $sh = '<a class="btn btn-mini-new btn-danger disabled  "  >已拥有</a>';
    }
    echo $sh . mb_substr($movie_info['movie_title'], 0, 70); ?>
    <div class="row movie"><!-- https://pics.javbus.info/sample/5rvz_11.jpg   https://jp.netcdn.space/ https://pics.dmm.co.jp/digital/video/ipz00865/ipz00865jp-12.jpg-->
      <div class="col-md-9 screencap">
        <a class="bigImage" href="<?php echo $picurl . $movie_info['movie_pic_cover']; ?>">
          <img class = "bigImagesrc" src="<?php echo $picurl . $movie_info['movie_pic_cover']; ?>"></a>
      ...
      <div class="col-md-3 info">
        ...
        <p>
          <a title="上一个识别码 {{ $last_censored_id }}" href="movie?censored_id={{ $last_censored_id }}"><span class="glyphicon glyphicon-chevron-left"></span>上一页 |</a><a title="下一个识别码 {{ $next_censored_id }}" href="movie?censored_id={{ $next_censored_id }}"> 下一页<span style = "height:20px" class="glyphicon glyphicon-chevron-right"></span></a> <a href = ''
            title="//游标分页 wait to do "> </a>
        ...
        <p><span class="header">javlibrary:</span>
          <a target="_blank" href="http://{{ $url_config['javlibhost'] }}/cn/<?php echo $movie_info['code_36'] ? '?v=' . $movie_info['code_36'] : 'vl_searchbyid.php?keyword=' . $movie_info['censored_id']; ?>">
            <span style=" "><?php echo $movie_info['censored_id'] . "({$movie_info['usersowned']})({$movie_info['userswanted']})({$movie_info['userswatched']})"; ?></span>
          </a>
        </p>
        ...
        <p><span class="header">预告片:</span>
          ...
          if (is_numeric(substr($pic_video, 0, 1))) {
              // 118abp00108/118abp00108pl.jpg
        ...
        <p>
          ...
          <a class="btn btn-mini-new btn-default" title = "清除访问记录 {{ $movie_info['visited'] }} " onclick="change_state('visited','0');$(this).html(0);">{{ $movie_info['visited'] }}</a>
        ...
        <ul>
          ...
          <div id="star_<?php echo $val['code_36']; ?>" class="star-box star-box-common star-box-up idol-box" style="left: 1047px; top: 326px; position: fixed; display: none;">
            <li>
              <a href="censored?st=<?php echo $val['code_36']; ?>"><img dsrc="https://jp.netcdn.space/mono/actjpgs/<?php echo $val['star_pic']; ?>" src = 'https://pics.javcdn.pw/actress/{{ $val['code_36'] }}_a.jpg' title=""></a>
...
//        }
    ...
    <a title="上一个识别码 {{ $last_censored_id }}" href="movie?censored_id={{ $last_censored_id }}"><span class="glyphicon glyphicon-chevron-left"></span> </a><a title="下一个识别码 {{ $next_censored_id }}" href="movie?censored_id={{ $next_censored_id }}"> <span style = "height:20px" class="glyphicon glyphicon-chevron-right"></span></a>
    <?php $sh = '';
    if ($movie_info['have_file'] == 3) {
        $sh = '<span style="color:red;">[已有文件]</span>';
    }
    echo $sh . $movie_info['censored_id'] . ' ' . $movie_info['movie_title']; ?>
